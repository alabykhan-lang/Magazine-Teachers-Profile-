  ...
  try{
    ...
    results.forEach((res, i) => {
      ...
      if(res.status === 'fulfilled' && res.value){
        const val = res.value;
        if(key === 'cfg') cfg = {...cfg, ...val};
        if(key === 'ls_settings'){
          lsSettings = val;
          applyLsColors(val);
          applyCustomCategories(val);
        }
        if(key === 'labels') labelOverrides = {...labelOverrides, ...val};
        if(key === 'section_order') sectionOrder = val;
        if(key === 'form_config') formConfig = val;
      }
    });
...
setTimeout(async () => {
  ...
  try {
    // Add a global timeout for the boot process
    await Promise.race([
      initCloudSync(),
    ...
    /* Ensure UI is rendered regardless of sync status */
    renderLandingCards();
