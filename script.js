...
function magOpeningFeaturePages(list){const pages=[];const find=fn=>list.find(s=>fn(s));const eic=find(magIsEicAddressSub),hist=find(magIsHistorySub),prop=find(s=>magOpeningSpeechKind(s)==='proprietor'),boy=find(s=>magOpeningSpeechKind(s)==='senior-boy'),girl=find(s=>magOpeningSpeechKind(s)==='senior-girl');if(eic)pages.push({type:'eic-address',sub:eic,sec:{key:'eic-address',label:'Editor-in-Chief Address',layout:'eic-address'}});if(hist)pages.push({type:'school-history',sub:hist,sec:{key:'school-history',label:'Brief History of the School',layout:'school-history'}});if(prop)pages.push({type:'proprietor-speech',sub:prop,sec:{key:'proprietor-speech',label:"Proprietor's Speech",layout:'proprietor-speech'}});if(boy)pages.push({type:'senior-boy-speech',sub:boy,sec:{key:'senior-boy-speech',label:"Senior Boy's Speech",layout:'senior-boy-speech'}});if(girl)pages.push({type:'senior-girl-speech',sub:girl,sec:{key:'senior-girl-speech',label:"Senior Girl's Speech",layout:'senior-girl-speech'}});return pages;}
function magRefPhoto(src,cls){return `<div class="mag-ref-photo ${cls||''}">${src?`<img src="${esc(src)}" alt="Portrait"/>`:'<div class="mag-silhouette"></div>'}</div>`;}
...
function renderReferenceOpeningPage(ctx,kind){
  ...
  if(kind==='eic-address')return shell(`<div class="mag-eic-layout"><main><div class="mag-ref-icon">&#9998;</div><div class="mag-ref-kicker">Editorial Chairman's Address</div><h1 class="mag-ref-title">From the<br/>Editor-in-Chief</h1><div class="mag-ref-rule"></div><div class="mag-ref-body">${esc(body)}</div></main><aside class="mag-eic-side">${magRefPhoto(photo,'mag-eic-photo')}<div class="mag-ref-name">${esc(name||'{editor_in_chief_name}')}</div><span class="mag-ref-role">${esc(role||'{editor_in_chief_role}')}</span><div class="mag-eic-quote"><b>&ldquo;</b>${esc(quote||'A school is not built by buildings alone, but by the vision, values and people who believe in its purpose.')}</div></aside></div>`);
  if(kind==='school-history')return shell(`<section class="mag-history-layout"><div class="mag-ref-icon">&#128214;</div><h1 class="mag-ref-title">Brief History<br/>of the School</h1><div class="mag-ref-rule"></div><div class="mag-history-body"><div class="mag-history-path"><i></i><i></i><i></i><i></i><i></i></div><main><div class="mag-ref-body">${esc(magSplitText(body,3)[0])}</div>${magRefPhoto(photo,'mag-history-photo')}${cap?`<div class="mag-ref-kicker" style="color:var(--mag-charcoal);letter-spacing:1px;">${esc(cap)}</div>`:''}<div class="mag-ref-body">${esc(magSplitText(body,3).slice(1).join('\n\n'))}</div></main></div></section>`);
  if(kind==='proprietor-speech')return shell(`<div class="mag-ref-icon">&#9812;</div><div class="mag-ref-kicker">Speech</div><h1 class="mag-ref-title">Proprietor's Speech</h1><div class="mag-ref-rule"></div><div class="mag-speech-prop"><aside>${magRefPhoto(photo,'mag-prop-photo')}<div class="mag-ref-name">${esc(name||'{proprietor_name}')}</div><span class="mag-ref-role">${esc(role||'{proprietor_title}')}</span></aside><main class="mag-ref-body mag-prop-columns">${esc(body)}</main></div>`);
  if(kind==='senior-boy-speech')return shell(`<div class="mag-senior-boy-top"><main><div class="mag-ref-kicker">Student Speech</div><h1 class="mag-ref-title">Senior Boy's Speech</h1><div class="mag-ref-rule"></div><div class="mag-ref-body">${esc(magSplitText(body,3)[0])}</div></main><aside>${magRefPhoto(photo,'mag-senior-photo')}<div class="mag-ref-name">${esc(name||'{senior_boy_name}')}</div><span class="mag-ref-role">${esc(role||'{senior_boy_title}')}</span></aside></div><div class="mag-wide-quote">${esc(quote||'Great schools are not measured by their buildings, but by the dreams they inspire and the leaders they raise.')}</div><div class="mag-ref-body mag-two-cols">${esc(magSplitText(body,3).slice(1).join('\n\n'))}</div>`);
  return shell(`<div class="mag-senior-girl"><aside>${magRefPhoto(photo,'mag-girl-photo')}<div class="mag-ref-name">${esc(name||'{senior_girl_name}')}</div><span class="mag-ref-role">${esc(role||'{senior_girl_title}')}</span></aside><main><div class="mag-ref-kicker">Student Speech</div><h1 class="mag-ref-title">Senior Girl's Speech</h1><div class="mag-ref-rule"></div><div class="mag-girl-lead">${esc(magSplitText(body,4)[0])}</div><div class="mag-ref-body mag-two-cols">${esc(magSplitText(body,4).slice(1,3).join('\n\n'))}</div></main></div><div class="mag-wide-quote">${esc(quote||'Let us continue to support one another, celebrate our differences, and rise together as strong, compassionate, and visionary leaders.')}</div><div class="mag-ref-body mag-girl-bottom">${magSplitText(body,4).slice(1).map(t=>`<p>${esc(t)}</p>`).join('')}</div>`);
}
