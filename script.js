...
/* ═══════════════════════════════════════════════════════════
   GALLERY BULK UPLOAD
═══════════════════════════════════════════════════════════ */
let bulkPhotos=[];/* [{dataURL, file, caption}] */
...
/* loadAll/saveAll defined above — no duplicate */
let subs=loadAll();
...
async function submitForm(){
  ...
  try{
    /* Save to local first */
    const local=JSON.parse(localStorage.getItem('me_subs')||'[]');
    local.push(sub);localStorage.setItem('me_subs',JSON.stringify(local));subs=local;
...
function deleteSubmission(id){if(!confirm('Permanently delete this submission?'))return;dbDeleteSubmission(id);subs=loadAll().filter(s=>String(s.id)!==String(id));localStorage.setItem('me_subs',JSON.stringify(subs));renderAdmin();}
