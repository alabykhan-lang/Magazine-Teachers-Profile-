...
async function dbLoadAll(){
  ...
  try{
    ...
    if(cached.length){subs=cached;const _d='HTTP:'+(e?.status||e?.code||'?')+' | '+(e?.message||String(e)).slice(0,80)+' | RT:'+(_subChannel?(_subChannel.state||'?'):'no-channel')+' | net:'+(isNetworkTrulyOffline()?'OFFLINE':'online');console.error('[CACHE FALLBACK]',_d,e);showSync('syncing','Cached: '+_d);return cached;}
